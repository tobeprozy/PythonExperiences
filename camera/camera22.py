...
#对每张图片，识别出角点，记录世界物体坐标和图像坐标
    ...
    cv2.imshow('img', img)
    cv2.waitKey()
    ret, corners = cv2.findChessboardCorners(gray,(8,8),0)
    print(ret,corners)
    #criteria:角点精准化迭代过程的终止条件
    criteria = (cv2.TERM_CRITERIA_EPS + cv2.TERM_CRITERIA_MAX_ITER, 30, 0.001)
    #执行亚像素级角点检测
    corners2 = cv2.cornerSubPix(gray,99,(11,11),(-1,-1),criteria)
